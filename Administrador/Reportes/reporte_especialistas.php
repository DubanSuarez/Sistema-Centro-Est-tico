                            ...
                            <div class="row mb-4">
                                <div class="row mb-4">
                                    <div class="col-md-6 text-md-start text-center">
                                        <h5 class="fw-bold EstiloLetraTarjeta">Reporte de Especialistas</h5>
                                        <small class="text-muted">
                                            Desempeño y productividad del personal especializado
                                        </small>
                                    </div>
...
SELECT 
    cp.Id,
    CONCAT(cp.Nombre,' ',cp.Apellido) AS Especialista,
    cp.Especialidad,
    COUNT(c.Id) AS TotalCitas,
    SUM(c.ValorTotal) AS TotalFacturado
FROM cita c
INNER JOIN servicioespecialista se ON se.Id = c.Id_ServicioEspecialista
INNER JOIN contratopersona cp ON cp.Id = se.Id_Especialista
$where
...
";
                            ...
                            <form method="GET" class="row g-3 mb-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="especialista" class="form-select">
                                        <option value="">Todos</option>
                                        <?php
                                        $esp = mysqli_query($conexion, "
            ...
            ");
                                        while ($e = mysqli_fetch_assoc($esp)):
                                            ?>
                                            <option value="<?= $e['Id'] ?>" <?= $especialista == $e['Id'] ? 'selected' : '' ?>>
                                                <?= $e['NombreCompleto'] ?>
                                            </option>
                                        <?php endwhile; ?>
                                    </select>
                                        ...
                                        <?php if (mysqli_num_rows($resultado) > 0): ?>
                                            ...
                                            <?php while ($row = mysqli_fetch_assoc($resultado)): ?>
                                                <tr>
                                                    ...
                                                    <td class="fw-bold">
                                                        $<?= number_format($row['TotalFacturado'], 0) ?>
                                                    </td>
                                                </tr>
                                            <?php endwhile; ?>
                                        <?php else: ?>
                            ...
                            <nav class="mt-3">
                                <ul class="pagination justify-content-center">
                                    ...
                                    <li class="page-item <?= $pagina <= 1 ? 'disabled' : '' ?>">
                                        <a class="page-link"
                                            href="?<?= http_build_query(array_merge($params, ['pagina' => $pagina - 1])) ?>">
                                            &laquo;
                                        </a>
                                    </li>
                                    ...
                                    <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
                                        <li class="page-item <?= $pagina == $i ? 'active' : '' ?>">
                                            <a class="page-link"
                                                href="?<?= http_build_query(array_merge($params, ['pagina' => $i])) ?>">
                                                <?= $i ?>
                                            </a>
                                        </li>
                                    <?php endfor; ?>
